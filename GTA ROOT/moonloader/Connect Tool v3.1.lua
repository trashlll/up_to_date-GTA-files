...
local serv = {
	...
	{
		projectname = "Revent Role Play", -- Название проекта
		title = "Revent RP",              -- Сокращённое название
		list = {        									 -- Сервера проекта
			   {
				   name = 'First',
				   ip = '213.32.112.224',
  ...
  -------------------------------------------------------------------------------
   ...
   --Чтобы сервера загружались из этого массива или, чтобы у вас работал шаблон
   --Требуется найти в поиске (CTRL+F) строчки: " downloadUrlToFile("https://api.kscripts.ru/getlistservers.php", "moonloader/config/KopnevScripts/servers.ctool") "
   --И удалить их. (Внимание! Этих строчек 2 штуки. Требуется удалить все две)
   --Внимание! Автообновление списка серверов, перестанет работать
   --При обновлении придется проделать все эти шаги зоново 
}
...
function alogin(id, title, text)
	...
	if id == 22100 and account_info ~= nil and getServTitle(ip, port.."") == "Revent RP" then
		if text:find("В поле Ключ:") then
			gauthcode.v = string.match(text, 'В поле Ключ: {ffff00}(.-){ffffff}')
			gadd_window.v = true
		end
	end
	...
	-- {FFFFFF}Регистрация | {ae433d}Создание пароля
	-- {FFFFFF}Авторизация | {ae433d}Ввод пароля

	-- if title == "{FFFFFF}Регистрация | {ae433d}Создание пароля" then
	-- 	print("aaa")
	-- end
	if title == "{FFFFFF}Регистрация | {ae433d}Создание пароля" and account_info == nil and getServTitle(ip, port.."") == "Evolve RP" then
		checkpass = true
	end
	if title == "{FFFFFF}Регистрация | {ae433d}Правила сервера (1/2)" and account_info == nil and getServTitle(ip, port.."") == "Evolve RP" and checkpass then
		checkpass = false
		savepass = savepas
		savepas = nil
		pass_window_state.v = true
	end
	if title == "{FFFFFF}Авторизация | {ae433d}Ввод пароля" and account_info ~= nil and getServTitle(ip, port.."") == "Evolve RP" then
		if autopass.v == true then
			if typeent.v == 0 then
				sampSendDialogResponse(id, 1, 0, account_info['user_password'])
				return false
	...
	if (id == 1 or id == 2) and account_info == nil and getServTitle(ip, port.."") == "Samp RP" then -- ID 2 - "Регистрация"
		checkpass = true
	end
	if id == 3 and account_info == nil and getServTitle(ip, port.."") == "Samp RP" and checkpass then -- Правила сервера
		checkpass = false
		savepass = savepas
		savepas = nil
		pass_window_state.v = true
...
function sampev.onShowDialog(id, style, title, b1,b2,text)
	...
	if title == "{FFFFFF}Ввод | {ae433d}Google Auth" and account_info ~= nil and account_info['gauth'] ~= "nil" and getServTitle(ip, port.."") == "Evolve RP" then
		if autogcode.v == true then
			if typeent.v == 0 then
				sampSendDialogResponse(id, 1, 0, genCode(account_info['gauth']))
				return false
...
function main()
	...
	if doesFileExist('samp.asi') then
		sampasi = true
		print('Обнаружен samp.asi, рекомендуется его удалить для стабильной работы.')
	end
		...
		lua_thread.create(function()
			...
			if not doesFileExist('moonloader/config/KopnevScripts/servers.ctool') then
				...
				if tJSON:sub(1,1) == '{' or tJSON:sub(1,1) == '[' then
					...
					SCM("Ошибка записи JSON конфига. Попробуйте еще раз")
				end
			else
				local fpath = "moonloader/config/KopnevScripts/servers.ctool"
...
if limgui then
	...
	function customServWindow()
		...
		imgui.Begin(u8'Поддерживаемые сервера', customServ_window, 2)
			imgui.BeginChild('##serverslistedit', imgui.ImVec2(535, 760), true) 
				imgui.Text(u8"Офицально поддерживаемые сервера:") imgui.SameLine(409) 
				if imgui.Button(u8"Синхронизировать") then
					if not doesDirectoryExist('moonloader/config/KopnevScripts/') then createDirectory('moonloader/config/KopnevScripts/') end
								...
								elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
									if not doesFileExist('moonloader/config/KopnevScripts/servers.ctool') then
										...
										if tJSON:sub(1,1) == '{' or tJSON:sub(1,1) == '[' then
											...
											f:write(tJSON)
											f:close()
											SCM("Ошибка синхронизации. Попробуйте еще раз. Код: Error sync")
										else
											SCM("Ошибка синхронизации. Попробуйте еще раз. Код: Error JSON")
										end
									else
										local fpath = "moonloader/config/KopnevScripts/servers.ctool"
										local f = io.open(fpath, "r")
										serv = decodeJson(f:read"*a")
										f:close()
										SCM("Сервера успешно синхронизированы")
									end
								end
							end
						end)
				...
				for i, v in ipairs(serv) do
					if imgui.CollapsingHeader(u8(v['projectname'])) then
						...
						imgui.BeginChild('child'..i, imgui.ImVec2(515, sizeY), true)
							if imgui.Button(u8"Добавить сервер к проекту: "..v['projectname'], imgui.ImVec2(495, 20)) then
								imgui.OpenPopup(u8'Добавление сервера к проекту '..v['projectname'])	
							end
							if imgui.BeginPopupModal(u8'Добавление сервера к проекту '..v['projectname'], _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
								imgui.Text(u8"Введите данные нового сервера:")
								imgui.NewLine()
								imgui.Text(u8"Название сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##nameserv"..i, customServ_AddServToProject_Name) imgui.PopItemWidth()
								imgui.Text(u8"ИП сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##ipserv"..i, customServ_AddServToProject_IP) imgui.PopItemWidth()
								imgui.Text(u8"Порт сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##portserv"..i, customServ_AddServToProject_Port) imgui.PopItemWidth()
								imgui.NewLine()
							...
							imgui.Separator()
							imgui.Text(u8'Имя сервера') imgui.NextColumn()
							imgui.Text(u8'ИП:Порт') imgui.NextColumn()
							...
							for i2, v2 in ipairs(v['list']) do
								...
								if imgui.Button(u8"Изменить##"..v2['name']..v['projectname']) then
									imgui.OpenPopup(u8'Изменение сервера: '..v['projectname'].." | "..v2['name'])
								...
								if imgui.BeginPopupModal(u8'Изменение сервера: '..v['projectname'].." | "..v2['name'], _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
									imgui.Text(u8"Введите данные сервера:")
									imgui.NewLine()
									imgui.Text(u8"Название сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##nameserv"..i, customServ_AddServToProject_Name) imgui.PopItemWidth()
									imgui.Text(u8"ИП сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##ipserv"..i, customServ_AddServToProject_IP) imgui.PopItemWidth()
									imgui.Text(u8"Порт сервера:  ") imgui.SameLine(180) imgui.PushItemWidth(240)  imgui.InputText("##portserv"..i, customServ_AddServToProject_Port) imgui.PopItemWidth()
								...
								imgui.SameLine()
								if imgui.Button(u8"Удалить##"..v2['name']..v['projectname']) then
									imgui.OpenPopup(u8'Удаление сервера: '..v['projectname'].." | "..v2['name'])
								end
								if imgui.BeginPopupModal(u8'Удаление сервера: '..v['projectname'].." | "..v2['name'], _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
									imgui.TextColored(imgui.ImVec4(1, 0, 0, 1), u8'Внимание!')
									imgui.Text(u8'Если вы удалите сервер, то автологин на данном сервер работать не будет')
									imgui.Text(u8'Вы действительно хотите удалить сервер?')
									imgui.NewLine()
									if imgui.Button(u8'Отмена', imgui.ImVec2(250, 25)) then
										imgui.CloseCurrentPopup()
									end
									imgui.SameLine()
									if imgui.Button(u8'Удалить', imgui.ImVec2(250, 25)) then
				...
				imgui.Text(u8"Пользавательские сервера:") 
				imgui.SameLine(409) 
				if imgui.Button(u8" Добавить проект  ") then
					imgui.OpenPopup(u8"Добавление нового проекта")
				...
				if imgui.BeginPopupModal(u8'Добавление нового проекта', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
					imgui.Text(u8"Введите данные нового проекта:")
					imgui.NewLine()
					imgui.Text(u8"Полное название:  ") imgui.SameLine(160) imgui.PushItemWidth(240)  imgui.InputText("##namepr", customServ_AddProject_projectname) imgui.PopItemWidth() 
					imgui.SameLine() ShowHelpMarker(u8"Пример: Arizona Role Play")
					imgui.Text(u8"Короткое название:  ") imgui.SameLine(160) imgui.PushItemWidth(240)  imgui.InputText("##ippr", customServ_AddProject_title) imgui.PopItemWidth()
					imgui.SameLine() ShowHelpMarker(u8"Пример: Arizona RP")
					imgui.NewLine()
					-------------------------------------------------------------------------------------------------
					imgui.Separator()
					imgui.NewLine()
					imgui.Text(u8"Данные диалога авторизации: ")
					imgui.NewLine()
					imgui.Text(u8"Ориентироваться на:") imgui.SameLine(160)
					imgui.PushItemWidth(260)
					imgui.Combo('##orient', customServ_Orient_Auth_INT, customServ_Orient)
					imgui.PopItemWidth()
					if customServ_Orient_Auth_INT.v == 0 then
						imgui.Text(u8"Введите ID диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputInt("##customServ_Orient_Auth_ID", customServ_Orient_Auth_ID,0) imgui.PopItemWidth()
					end
					if customServ_Orient_Auth_INT.v == 1 then
						imgui.Text(u8"Заголовок диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_Auth_title", customServ_Orient_Auth_title) imgui.PopItemWidth()
					end
					if customServ_Orient_Auth_INT.v == 2 then
						imgui.Text(u8"Введите текст диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_Auth_text", customServ_Orient_Auth_text) imgui.PopItemWidth()
					end
					imgui.NewLine()
					-------------------------------------------------------------------------------------------------
					imgui.Separator()
					imgui.NewLine()
					imgui.Text(u8"Данные диалога GAuth: ")
					imgui.NewLine()
					imgui.Text(u8"Ориентироваться на:") imgui.SameLine(160)
					imgui.PushItemWidth(260)
					...
					if customServ_Orient_GAuth_INT.v == 0 then
						imgui.Text(u8"Введите ID диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputInt("##customServ_Orient_GAuth_ID", customServ_Orient_GAuth_ID,0) imgui.PopItemWidth()
					end
					if customServ_Orient_GAuth_INT.v == 1 then
						imgui.Text(u8"Заголовок диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_GAuth_title", customServ_Orient_GAuth_title) imgui.PopItemWidth()
					end
					if customServ_Orient_GAuth_INT.v == 2 then
						imgui.Text(u8"Введите текст диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_GAuth_text", customServ_Orient_GAuth_text) imgui.PopItemWidth()
					end
					imgui.NewLine()
					-------------------------------------------------------------------------------------------------
					imgui.Separator()
					imgui.NewLine()
					imgui.Text(u8"Данные диалога Пин-Код: ")
					imgui.NewLine()
					imgui.Text(u8"Ориентироваться на:") imgui.SameLine(160)
					imgui.PushItemWidth(260)
					...
					if customServ_Orient_Pin_INT.v == 0 then
						imgui.Text(u8"Введите ID диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputInt("##customServ_Orient_Pin_ID", customServ_Orient_Pin_ID,0) imgui.PopItemWidth()
					end
					if customServ_Orient_Pin_INT.v == 1 then
						imgui.Text(u8"Заголовок диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_Pin_title", customServ_Orient_Pin_title) imgui.PopItemWidth()
					end
					if customServ_Orient_Pin_INT.v == 2 then
						imgui.Text(u8"Введите текст диалога:") imgui.SameLine(160) imgui.PushItemWidth(260)  imgui.InputText("##customServ_Orient_Pin_text", customServ_Orient_Pin_text) imgui.PopItemWidth()
					end
					imgui.NewLine()
					-------------------------------------------------------------------------------------------------
					if imgui.Button(u8"Закрыть", imgui.ImVec2(200,20)) then
				...
				for i, v in ipairs(customServ) do
					if imgui.CollapsingHeader(v['projectname']) then
						...
						imgui.BeginChild('child2'..i, imgui.ImVec2(515, sizeY), true)
							if imgui.Button(u8"Добавить сервер к проекту", imgui.ImVec2(240, 20)) then
								imgui.OpenPopup(u8'Добавление сервера к проекту '..v['projectname'])	
							...
							imgui.SameLine()
							if imgui.Button(u8"Редактировать проект", imgui.ImVec2(240, 20)) then
								imgui.OpenPopup(u8'Редактирование проекта: '..v['projectname'])	
							...
							if imgui.BeginPopupModal(u8'Редактирование проекта: '..v['projectname'], _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
								imgui.Text(u8"Введите данные проекта:")
								imgui.NewLine()
								imgui.Text(u8"Полное название:  ") imgui.SameLine(160) imgui.PushItemWidth(240)  imgui.InputText("##namepr", customServ_AddProject_projectname) imgui.PopItemWidth() 
								imgui.SameLine() ShowHelpMarker(u8"Пример: Arizona Role Play")
								imgui.Text(u8"Короткое название:  ") imgui.SameLine(160) imgui.PushItemWidth(240)  imgui.InputText("##ippr", customServ_AddProject_title) imgui.PopItemWidth()
	...
	function mainwindow()
		...
		if vkladki[1] == true then -- Менеджер аккаунтов
			...
			if selacc > -1 then
				...
				imgui.SameLine()
				if imgui.Button(fa.ICON_TRASH.. u8'  Удалить', imgui.ImVec2(110, 27)) then imgui.OpenPopup(u8"Подтверждение") end imgui.SameLine()
		...
		if vkladki[2] == true then -- Реконнект
				...
				end imgui.SameLine() ShowHelpMarker(u8'"Пингануть" - если на протяжении долгого времени, у вас не подключается к серверу (Server didn\'t resopnse), то нажмите эту кнопку') imgui.NewLine()  --fa.ICON_REFRESH
		...
		if vkladki[3] == true then -- Доп Коды
			...
			if selacc ~= -1 then
				...
				imgui.SameLine()
				if imgui.Button(fa.ICON_TRASH.. u8'  Удалить', imgui.ImVec2(120, 30)) then imgui.OpenPopup(u8"Подтверждение") end
				imgui.SameLine()
			end
			imgui.NewLine() imgui.NewLine() 
			imgui.Text(u8'Сортировка: ') imgui.SameLine()
		...
		if vkladki[4] == true then -- Настройки
			...
			if imgui.Button(u8'Авто-логин', imgui.ImVec2(100, 25)) then 
				imgui.OpenPopup(u8"Настройки Авто-Логина")
			end
			imgui.SetCursorPos(imgui.ImVec2(523, 261))
			if imgui.Button(u8'Сервера', imgui.ImVec2(100, 25)) then 
	...
	function popup_changelog()
		if imgui.BeginPopupModal(u8'Список изменений', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + 1) then
			imgui.SameLine(150)
			...
			imgui.BeginGroup()
				imgui.BeginChild("changelog", imgui.ImVec2(380,400), true)
					...
					imgui.SameLine(15) imgui.Text(u8'1. Исправлены авторизации для некоторых серверов.') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'2. Вырезан лишний мусор со скрипта. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'3. Добавлена нормальная поддержка доменов. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'4. Обновлена ссылка для синхронизации серверов. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'Чтобы синхронизировать поддерживаемые сервера') imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'перейдите в Настройки -> Сервера -> Синхронизировать') imgui.NewLine() imgui.NewLine()
					...
					imgui.SameLine(15) imgui.Text(u8'1. Добавлена возможность добавлять свои проекты. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'2. Можно редактировать поддерживаемые проекты. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'3. Добавлены Popup\'ы. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'4. Поддерживаемы сервера теперь авматически не синхр. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'Чтобы синхронизировать поддерживаемые сервера') imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'перейдите в Настройки -> Сервера -> Синхронизировать') imgui.NewLine() imgui.NewLine()
					imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
					imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
					...
					imgui.SameLine(15) imgui.Text(u8'1. Исправлен баг со слётом конфига. ') imgui.NewLine() imgui.NewLine()
					imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
					imgui.Text(u8'V 2.97') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'1. Добавлена поддержка сервера Revent Role Play. ') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'2. Сервера Diamond RP определяются лучше.') imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) imgui.Text(u8'3. Оптимизация покинула чат.') imgui.NewLine() imgui.NewLine() 
					imgui.SameLine(15) imgui.Text(u8'V2.98 - Оптимизация зашла в чат.') imgui.NewLine() imgui.NewLine()
					imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
					...
					imgui.SameLine(15) imgui.Text(u8'2. К серверам Diamond RP - присоединится стало проще.') imgui.NewLine() 
					imgui.SameLine(15) imgui.Text(u8'(исправлено Server didn\'t respond).') imgui.NewLine() imgui.NewLine()
					imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
				imgui.EndChild()
			imgui.EndGroup()
			imgui.SetCursorPos(imgui.ImVec2(300, 460))
	...
	function popup_autologin()
		if imgui.BeginPopupModal(u8'Настройки Авто-Логина', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + 1) then
			imgui.SameLine(130)
			imgui.Text(u8'Настройки авто-логина')
			imgui.NewLine()
			imgui.NewLine()
	...
	function popup_delete()
		if imgui.BeginPopupModal(u8'Подтверждение', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + 1) then
			imgui.TextColored(imgui.ImVec4(1, 0, 0, 1), u8'Внимание!')
			...
			imgui.NewLine()
			imgui.SameLine(210) if imgui.Button(u8'Отмена', imgui.ImVec2(100, 25)) then imgui.CloseCurrentPopup() end imgui.SameLine()
    ...
    if account_info == nil then
        lua_thread.create(function()
            ...
            for i, v in ipairs(accounts) do
                ...
                if (server_ip == ip2 or server_ip == ip) and v['server_port'] == port.."" and v['user_name'] == name then
                    print('{00FF00}Успешно{FFFFFF} загружен аккаунт {2980b9}'..v['user_name']..'{FFFFFF}.')
                    SCM('Аккаунт загружен со второй попытки. Если диалог открыт, нажмите "enter"')
                    account_info = v
                    break
                end
            end
...
function save_customServ()
	...
	local fpath =  getWorkingDirectory().."\\config\\KopnevScripts\\customServ.json"
		...
		SCM("Ошибка записи JSON Config. Попробуйте повторить попытку сохранения. (Config - customServ.json)")
	end
	local tJSON = encodeJson(serv)
	if tJSON:sub(1,1) == '{' or tJSON:sub(1,1) == '[' then
	local fpath =  getWorkingDirectory().."\\config\\KopnevScripts\\servers.ctool"
		...
		SCM("Ошибка записи JSON Config. Попробуйте повторить попытку сохранения. (Config - servers.ctool)")
	end
end
...
--             else sampAddChatMessage("[Connect Tool] Внутренняя ошибка. Попробуйте позже", 0xFF0000) print(res) end
--         end
--     end)
-- end
